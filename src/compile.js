...
var ensureJsASTStatements = function (nodes) {
  if (typeof nodes.length !== 'undefined') {
    ...
    throw new Error(
      "ensureJsASTStatements wasn't given an Array, got " +
        nodes +
        ' (' +
        typeof nodes +
        ')',
...
var compileNodeWithEnvToJsAST = function (n, env, opts) {
  ...
  var result = n.accept({
    ...
    visitImportIntoModule: function () {
      ...
      var imported =
        env.$importCache && env.$importCache[modulePathKey]
        ...
        _.each(n.liftedIds, function (id) {
          ...
          if (isOperator(id)) {
            ...
            _.each(cached.exports, function (sym, symName) {
              if (
                sym instanceof types.FunctionType &&
    ...
    visitFunction: function () {
      ...
      if (
        n.body === null ||
        (Array.isArray(n.body) && n.body.length === 1 && n.body[0] === null)
      ) {
        throw new Error(
          "Function '" +
            (n.name || '<anonymous>') +
    ...
    visitCall: function () {
      ...
      let isConstructor =
        funcType &&
        funcType instanceof types.FunctionType &&
...
const compile = function (source, env, aliases, opts) {
        ...
        if (ann instanceof nodes.FuncAnnotation) {
          if (ann.name === 'extern') {
            ...
            if (!(n instanceof nodes.Function)) {
              throw new Error(
                'extern annotation can only be applied to functions at line ' +
                  n.lineno,
              );
            }
            ...
            if (
              currentFunc.body &&
              currentFunc.body.length == 1 &&
              currentFunc.body[0] !== null
            ) {
              throw new Error(
                'extern functions must not have a body at line ' + n.lineno,
              );
            }
            ...
            if (missingTypes.length > 0) {
              throw new Error(
                "extern function '" +
                  ...
                  "' at line " +
                  n.lineno +
                  ' has arguments without type annotations: ' +
                  missingTypes.join(', '),
              );
            }
  ...
  return {
    ast: ast,
    type: resultType,
    output: escodegen.generate(ensureJsASTStatement(jsAst), {
      comment: true,
    }),
  };
};
