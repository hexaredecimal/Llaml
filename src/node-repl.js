...
var nodeRepl = function (opts) {
  ...
  vm.runInNewContext(
    compile(prelude, env, {}, { nodejs: true }).output,
    sandbox,
    'eval',
  );
  repl.setPrompt('> ');
  ...
  repl.on('line', function (line) {
    ...
    try {
      if (!inBlock && /^:/.test(metacommand[0])) {
        ...
        // Just eval it
        compiled = compile(joined, env, aliases, {
          nodejs: true,
          filename: '.',
          run: true,
        });
...
var runRoy = function (argv, opts) {
  ...
  _.each(argv, function (filename) {
    ...
    exported = {};
    const outputPath = path.basename(filename.replace(extensions, '.js'));
...
var processMeta = function (commands, env, aliases, sources) {
  ...
  switch (commands[0]) {
    ...
    case ':l':
      ...
      return compile(source, env, aliases, {
        nodejs: true,
        filename: '.',
        run: true,
      });
    ...
    case ':s':
      ...
      if (sources[commands[1]]) {
        ...
        if (commands[1]) {
          ...
          console.log(
            ':s [identifier] :: show original code about identifier.',
          );
        }
      }
      break;
